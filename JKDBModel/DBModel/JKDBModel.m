...
- (BOOL)saveOrUpdate
{
    id object = [self.class findByPK:[self primaryValue]];
    if (object) {
       return [self save];
    }
    return [self update];
...
- (BOOL)save
{
    ...
    for (int i = 0; i < self.columeNames.count; i++) {
        ...
        if ([proname isEqualToString:primaryId]) {
            ...
            if ([JKDBModel isNullString:value]) {
                NSLog(@"ERROR:插入失败,主键值为空");
                return NO;
            }
...
- (BOOL)update
{
    ...
    if ([JKDBModel isNullString:primaryValue]) {
        NSLog(@"更新失败，主键值为空");
        return NO;
    }
...
+ (BOOL)updateObjects:(NSArray *)array
{
    ...
    [jkDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (JKDBModel *model in array) {
            ...
            id primaryValue = [model primaryValue];
            if ([JKDBModel isNullString:primaryValue]) {
                res = NO;
                *rollback = YES;
                return;
...
- (BOOL)deleteObject
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        NSString *tableName = NSStringFromClass(self.class);
        id primaryValue = [self primaryValue];
        if ([JKDBModel isNullString:primaryValue]) {
            return ;
        }
...
+ (BOOL)isNullString:(NSString *)object
{
